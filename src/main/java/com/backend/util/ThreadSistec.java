...
@AllArgsConstructor
@NoArgsConstructor
public class ThreadSistec extends Thread{
	
	private MultipartFile sistecFile;
	
	private String sistecDATA;
	...
	public void run() {
		...
		try {
			...
			InputStream input = sistecFile.getInputStream();
			BufferedReader csv = new BufferedReader(new InputStreamReader(input, "UTF-8"));
			
			String line = csv.readLine();
			line = csv.readLine();
			
			List<String> columns = ColumnFile.getColumnsSistec();
			...
			while(line != null) {
				...
				obj.put(columns.get(21), arrCSV[21]); 
				obj.put(columns.get(22), arrCSV[22]); 
				obj.put(columns.get(23), arrCSV[23]);
				obj.put(columns.get(24), arrCSV[24]); 
			...
			this.sistecDATA += buffer;
			System.out.println("ARQUIVO SISTEC LIDO COM SUCESSO!");
			
		} catch (IOException e) {
			System.out.println("ERRO AO LER AQUIVO DO SISTEC!");
		}
